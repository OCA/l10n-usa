...
class CacheManager:
    ...
    def __init__(self, env, ttl_hours: int = 720):
        ...
        self.Cache = env['us.tax.api.cache']
    ...
    def set(self, address_hash: str, provider_id: int, rates: dict,
            zip_code: str = None, state_id: int = None,
        ...
        try:
            ...
            _logger.debug('Cache SET for hash=%s, TTL=%sh', address_hash[:12], self.ttl_hours)
        except Exception as exc:
            _logger.warning('Cache write failed: %s', exc)

    def build_hash(self, zip_code: str, state_code: str,
                   product_category_code: str, date) -> str:
        date_month = str(date)[:7] if date else 'nodate'  # 'YYYY-MM'
        return self.Cache.build_hash(zip_code, state_code, product_category_code, date_month)
