...
from .address_resolver import resolve_shipping_address, is_us_address
...
class UsTaxEngineService(models.AbstractModel):
    ...
    @api.model
    def _process(self, res_model, res_id, address, lines,
                 doc_date, company_id, currency_id, apply_fn):
        ...
        if not zip_code or not state_code:
            _logger.warning('Tax engine: missing ZIP or state for %s %s', res_model, res_id)
        ...
        # Step 3: Check nexus
        has_nexus = self.env['us.tax.nexus'].has_nexus(company_id, state.id if state else False)
        if not has_nexus:
            self._log(res_model, res_id, address, 'exempt_nexus',
        ...
        # Pre-load TANGIBLE category for fallback (products without category = taxable goods)
        ...
        for line in lines:
            ...
            if not taxable:
                results.append({'line_id': line.id, 'source': 'exempt_rule', 'tax': 0.0})
                continue
    ...
    @api.model
    def _get_rate(self, zip_code, state_code, state_id, product_category_code,
                  product_category_id, doc_date, address, cache,
                  engine_mode, fail_policy, rate_override=None):
        """Return rate dict from best available source."""

        if rate_override is not None:
            return {
                'total_rate': rate_override, 'state_rate': rate_override,
                'county_rate': 0, 'city_rate': 0, 'district_rate': 0,
                'source': 'manual',
            }
        ...
        # Cache check
        cache_hash = cache.build_hash(zip_code, state_code, product_category_code, doc_date)
        ...
        for provider_rec in providers:
            ...
            try:
                ...
                rates['provider_id'] = provider_rec.id if provider_rec.code != 'local' else False
                # Save to cache only for external providers
                if provider_rec.code != 'local':
                    cache.set(
                        cache_hash, provider_rec.id, rates,
                        zip_code=zip_code, state_id=state_id,
        ...
        return self._handle_all_failed(
            fail_policy, cache_hash, last_error, address
        )
    ...
    def _handle_all_failed(self, fail_policy, cache_hash, last_error, address):
        ...
        _logger.warning('US Tax: all providers failed, applying 0 tax. Policy=%s', fail_policy)
        return {
            'total_rate': 0.0, 'state_rate': 0.0, 'county_rate': 0.0,
            'city_rate': 0.0, 'district_rate': 0.0, 'source': 'error',
        }
    ...
    def _apply_to_sale_order(self, order, result):
        ...
        if order_source in ('exempt_nexus', 'disabled', 'skip_non_us', 'skip_no_address'):
    ...
    def _apply_to_invoice(self, move, result):
        ...
        state_code = (
            ...
            if (hasattr(move, 'partner_shipping_id')
    ...
    def _get_or_create_state_tax(self, state_code, company, rate_pct=0.0):
        """Get or create an account.tax for this US state with the exact calculated rate.
        ...
        if not tax:
            ...
            tax = Tax.create({
                ...
                'description': f'US Sales Tax — {state_code} @ {amount_pct:.4g}% (US Tax Engine)',
            })
    ...
    @api.model
    def _log(self, res_model, res_id, address, source,
             taxable_amount=0, tax_amount=0, provider_id=None,
             state_id=None, nexus_applied=False, total_rate=0,
             state_rate=0, county_rate=0, partner_id=None):
        # Resolve state_id from address if not provided
        if not state_id and address.get('state'):
            state = self.env['res.country.state'].search(
                [('code', '=', address['state']), ('country_id.code', '=', 'US')], limit=1
            )
            state_id = state.id if state else False
