...
def migrate(cr, version):
    ...
    for view_id, view_name in views:
        try:
            cr.execute(
                "SELECT arch_db::text FROM ir_ui_view WHERE id = %s",
                (view_id,)
            )
            row = cr.fetchone()
            ...
            if _CONTROL_CHAR_RE.search(arch_str):
                ...
                _logger.debug(
                    ...
                    view_id, (view_name or '')[:60]
                )
        except Exception as exc:
            _logger.warning(
                'Could not clean view id=%s: %s', view_id, exc
            )

    if fixed:
        _logger.info(
            'l10n_us_sales_tax_engine pre-migrate: '
            'cleaned %d views with XML-invalid control characters.', fixed
        )
    else:
        _logger.info(
            'l10n_us_sales_tax_engine pre-migrate: '
