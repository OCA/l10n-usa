...
class AccountBankingMandate(models.Model):
    ...
    def _compute_display_name(self):
        for mandate in self:
            ...
            if mandate.format == "ach":
                name += f" ({mandate.recurrent_sequence_type})"
    ...
    def _achdd_mandate_set_state_to_expired(self):
        logger.info(
            self.env._("Searching for ACH Mandates that must be set to Expired")
        )
        expire_limit_date = fields.Date.today() + relativedelta(
        ...
        if expired_mandates:
            ...
            logger.info(self.env._("0 ACH Mandates had to be set to Expired"))
        return True
