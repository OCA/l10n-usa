...
class TestNexus(UsTaxBaseTest):
    ...
    def test_nexus_exists_for_fl(self):
        ...
        has = self.env['us.tax.nexus'].has_nexus(
            self.env.company.id, self.fl.id
        )
        self.assertTrue(has)
    ...
    def test_no_nexus_for_ny(self):
        ...
        has = self.env['us.tax.nexus'].has_nexus(
            self.env.company.id, self.ny.id
        )
...
class TestProductCategory(UsTaxBaseTest):
    ...
    def test_exempt_not_taxable(self):
        ...
        taxable, _ = self.env['us.tax.rule'].is_taxable(
            self.fl.id, self.cat_exempt.id
        )
