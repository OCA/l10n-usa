...
class UsTaxController(http.Controller):
    ...
    def calculate(self, **kwargs):
        ...
        try:
            ...
            state_code = body.get('state', '').strip()
            amount = float(body.get('amount', 0))
            category = body.get('product_category', 'TANGIBLE')
            calc_date = body.get('date') or str(date.today())
    ...
    @http.route('/api/v1/us_tax/rate_lookup', type='json', auth='user', methods=['GET', 'POST'])
    def rate_lookup(self, **kwargs):
        ...
        try:
            ...
            calc_date = body.get('date', str(date.today()))

            zip_norm = normalize_zip(zip_code)
            mapping = request.env['us.tax.zip.mapping'].get_best_jurisdiction(
                zip_norm, state_code=state_code
            )
            if not mapping:
                return {'zip': zip_code, 'state': state_code, 'found': False,
                        'message': 'No local jurisdiction mapping found.'}

            rate = request.env['us.tax.rate'].get_rate_for_date(mapping.id, calc_date)
            if not rate:
                return {'zip': zip_code, 'state': state_code, 'found': False,
