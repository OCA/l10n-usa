...
class UsTaxRate(models.Model):
    ...
    state_id = fields.Many2one(
        related='jurisdiction_id.state_id', store=True, index=True,
    )
    ...
    total_rate = fields.Float(
        digits=(5, 4), compute='_compute_total_rate', store=True,
    ...
    source = fields.Char(
        default='manual',
        help='Origin: florida_dor, texas_open_data, api_ziptax, api_taxjar, manual',
    )
    ...
    def _check_dates(self):
        for rec in self:
            if rec.end_date and rec.effective_date > rec.end_date:
                raise ValidationError(self.env._('Effective date must be before or equal to end date.'))
    ...
    def get_rate_for_date(self, jurisdiction_id, date, product_category_id=None):
        ...
        if product_category_id:
            ...
            specific = self.search(
                domain + [('product_tax_category_id', '=', product_category_id)],
