...
class UsTaxBaseTest(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # Enable engine
        cls.env['ir.config_parameter'].sudo().set_param('l10n_us_tax.engine_active', 'True')
        cls.env['ir.config_parameter'].sudo().set_param('l10n_us_tax.engine_mode', 'hybrid')
        cls.env['ir.config_parameter'].sudo().set_param('l10n_us_tax.fail_policy', 'warn')
