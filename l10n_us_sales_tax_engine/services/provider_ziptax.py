...
ZIPTAX_SANDBOX_URL = 'https://api.zip-tax.com/request/v40'  # Same endpoint, key controls access
...
class ProviderZipTax(ProviderBase):
    ...
    def validate_credentials(self) -> bool:
        try:
            ...
            resp = requests.get(
                ZIPTAX_BASE_URL,
                params={'key': key, 'zip': '33101', 'country': 'US'},
    ...
    def get_rate(self, payload: dict) -> dict:
        ...
        _logger.info('ZipTax API call for ZIP=%s (key=%s)', zip_code, self._mask_key(key))

        try:
            resp = requests.get(
                ZIPTAX_BASE_URL,
                params={'key': key, 'zip': zip_code, 'country': 'US'},
                timeout=self.timeout,
            )
            resp.raise_for_status()
        except requests.Timeout:
            raise ProviderError(f'ZipTax: request timed out after {self.timeout}s.')
        except requests.HTTPError as exc:
            raise ProviderError(f'ZipTax HTTP error: {exc}')

        self.record.increment_call_counter()
        raw = resp.json()

        if not raw.get('results'):
            raise ProviderError(f'ZipTax: no results for ZIP={zip_code}. rCode={raw.get("rCode")}')
    ...
    def normalize_response(self, raw: dict) -> dict:
        return {
            ...
            'county_rate':   float(raw.get('countyTaxRate', 0)),
            'city_rate':     float(raw.get('cityTaxRate', 0)),
            'district_rate': float(raw.get('combinedDistrictTaxRate', 0)),
            'total_rate':    float(raw.get('combinedSalesTaxRate', 0)),
            'source_date':   None,
            'raw_response':  raw,
        }
