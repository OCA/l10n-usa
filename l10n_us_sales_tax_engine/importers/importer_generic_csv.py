    ...
    def run(self, file_obj, state, effective_date):
        ...
        for row_num, row in enumerate(reader, start=2):
            try:
                ...
                county = (row.get(self.column_map.get('county', ''), '') or '').strip().upper()
                city = (row.get(self.column_map.get('city', ''), '') or '').strip().upper()

                def _rate(key):
                    val = row.get(self.column_map.get(key, key), '0') or '0'
                ...
                total_rate = _rate('total_rate') or (
                    _rate('state_rate') + _rate('county_rate') +
                    _rate('city_rate') + _rate('district_rate')
                )
                rates = {
                ...
                self._upsert_zip_mapping(
                    ...
                    confidence=0.9, source=self.SOURCE_CODE,
                )
                if action == 'created':
                    created += 1
                else:
        ...
        _logger.info('Generic CSV import done: %d created, %d updated, %d skipped',
