...
class UsTaxProvider(models.Model):
    ...
    code = fields.Char(
        required=True, size=32, index=True,
        help='Unique provider code: local, ziptax, taxjar, api_ninjas',
    )
    ...
    limit_status = fields.Selection([
        ...
        ('unlimited', 'Unlimited'),
    ], compute='_compute_limit_status', store=False)
    ...
    def is_limit_warning(self) -> bool:
        """Return True if this provider is at ≥80% of its monthly limit."""
        self.ensure_one()
        if self.monthly_call_limit == 0:
            return False
        return self.calls_this_month >= (self.monthly_call_limit * LIMIT_WARNING_THRESHOLD)
    ...
    def increment_call_counter(self):
        ...
        self.sudo().write({'calls_this_month': new_count})
        ...
        if self.monthly_call_limit > 0:
            ...
            if pct >= LIMIT_WARNING_THRESHOLD and not self.limit_alert_sent:
                self.sudo().write({'limit_alert_sent': True})
            ...
            if new_count >= self.monthly_call_limit:
                _logger.warning(
                    ...
                    self.name, new_count, self.monthly_call_limit
                )
    ...
    def _send_limit_warning_notification(self, current_count):
        ...
        try:
            group = self.env.ref(
                'l10n_us_sales_tax_engine.group_us_tax_manager', raise_if_not_found=False
            )
            if group:
    ...
    def _get_provider_service(self):
        ...
        from ..services import (  # noqa: PLC0415
            provider_local, provider_ziptax,
            provider_api_ninjas, provider_taxjar,
        )
        mapping = {
        ...
        if not cls:
            raise UserError(self.env._('No service class found for provider code "%s".', self.code))
        return cls
