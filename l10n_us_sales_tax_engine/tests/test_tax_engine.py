...
class TestTaxEngine(UsTaxBaseTest):
    ...
    def test_engine_disabled_returns_skip(self):
        ...
        result = self.env['us.tax.engine.service']._process(
            'sale.order', 0, {'zip': '33101', 'state': 'FL', 'country_code': 'US'},
            [], None, self.env.company.id, self.env.company.currency_id.id,
    ...
    def test_no_nexus_returns_exempt(self):
        ...
        result = self.env['us.tax.engine.service']._process(
            ...
            [], '2025-01-01', self.env.company.id, self.env.company.currency_id.id,
            apply_fn=lambda r: None,
        )
    ...
    def test_local_rate_used_for_fl_zip(self):
        ...
        result = engine._get_rate(
            zip_code='33101', state_code='FL', state_id=self.fl.id,
            product_category_code='TANGIBLE', product_category_id=self.cat_tangible.id,
            doc_date='2025-01-01', address={'zip': '33101', 'state': 'FL'},
            cache=cache, engine_mode='hybrid', fail_policy='warn',
        )
    ...
    def test_cache_hit_on_second_call(self):
        ...
        provider = self.env['us.tax.provider'].search([('code', '=', 'ziptax')], limit=1)
        cache = CacheManager(self.env, ttl_hours=720)  # noqa: F841
        h = cache.build_hash('33101', 'FL', 'TANGIBLE', '2025-01-01')
        cache.set(h, provider.id, {
            'total_rate': 0.07, 'state_rate': 0.06,
            'county_rate': 0.01, 'city_rate': 0, 'district_rate': 0,
        })
        result = cache.get(h)
