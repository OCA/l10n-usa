...
class TestBillcomConfig(BillcomTestCommon):
    ...
    def test_config_api_url_computation(self):
        ...
        self.billcom_config._compute_api_url()
        self.assertEqual(
            self.billcom_config.api_url, "https://gateway.prod.bill.com/connect"
        )
    ...
    def test_connection_test_failure(self):
        ...
        try:
            with patch(
                ...
                # Call test_connection and expect UserError
                try:
                    self.billcom_config.test_connection()
                    self.fail("Expected UserError to be raised")
                except UserError as e:
                    # Verify error message
                    self.assertIn("Authentication failed", str(e))

                # Verify error state was set (inside same transaction)
                self.billcom_config.invalidate_recordset()
                self.assertEqual(self.billcom_config.state, "error")
                self.assertIn(
                    "Authentication failed", self.billcom_config.last_error_message
                )
        finally:
