...
class ProviderApiNinjas(ProviderBase):
    ...
    def get_rate(self, payload: dict) -> dict:
        ...
        _logger.info('API Ninjas call for ZIP=%s (key=%s)', zip_code, self._mask_key(key))
        ...
        try:
            ...
            raise ProviderError(f'API Ninjas: request timed out after {self.timeout}s.')
        except requests.HTTPError as exc:
            raise ProviderError(f'API Ninjas HTTP error: {exc}')
