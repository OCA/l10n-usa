...
class UsTaxCalculationLog(models.Model):
    ...
    product_category_id = fields.Many2one('us.tax.product.category', ondelete='set null')
    ...
    calculated_by = fields.Many2one(
        'res.users', default=lambda s: s.env.user, ondelete='set null',
    )
        ...
        for rec in self:
            if rec.res_model and rec.res_id:
                try:
                    ...
                    rec.document_ref = doc.display_name or f'{rec.res_model},{rec.res_id}'
                except Exception:
                    rec.document_ref = f'{rec.res_model},{rec.res_id}'
            else:
                rec.document_ref = ''
    ...
    def log_calculation(self, payload):
        ...
        for key in ('request_payload', 'response_payload'):
            if key in payload and isinstance(payload[key], dict):
                sanitized = {k: v for k, v in payload[key].items()
