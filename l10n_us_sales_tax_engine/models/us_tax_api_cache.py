...
class UsTaxApiCache(models.Model):
    ...
    @api.model
    def create_or_update(self, address_hash, provider_id, rates, ttl_hours,
                         request_payload=None, response_payload=None,
                         zip_code=None, state_id=None):
        """Upsert a cache entry."""
        now = fields.Datetime.now()
        expires = now + timedelta(hours=ttl_hours) if ttl_hours else False
